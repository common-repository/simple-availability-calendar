<?php
	...
	if (is_super_admin()) {
		...
		$hidGET = '';
		$exclKeys = array('user_id', 'paged', 'show_all');
		foreach($_GET as $key => $val) {
			if ( ! in_array($key, $exclKeys) )
...
<h1><?php echo __('Users appointments', 'sa-calendar'); ?></h1>
<form method="get" action="<?php menu_page_url('sa_calendar_userapps') ?>">
	<?php echo $hidGET; ?>
...
<?php endif; ?>
<?php if ($user_id > 0): ?>
		<h2 style="margin-top: 50px"><?php echo sprintf(__('Appointments of %s', 'sa-calendar'), $user_name); ?></h2>
		<div style="max-width: 1000px">
		<?php echo do_shortcode('[sa_calendar_userapps for_user="' . $user_id . '"]'); ?>
		</div>
<?php endif; ?>
</div>